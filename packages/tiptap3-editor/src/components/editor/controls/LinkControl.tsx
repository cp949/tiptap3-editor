...
export const LinkControl = (props: ControlProps) => {
  ...
  }, [open, editor, setActiveToolbarPopup]);
  ...
  return (
    <Popover.Root open={open} onOpenChange={setOpen}>
      <Popover.Trigger asChild>
        <Control isActive={editor?.isActive("link")} title="Link" {...props}>
          <LinkIcon size={18} />
        </Control>
      </Popover.Trigger>
      <Popover.Portal>
        ...
        >
          <div className="te-flex te-flex-col te-gap-1.5">
            <label
              htmlFor={textId}
              className="te-text-xs te-font-medium te-text-gray-500"
            >
              Link Text
            </label>
            <input
          ...
          <div className="te-flex te-items-center te-justify-start te-gap-3 te-mt-1">
            <ActionButton variant="primary-outline" onClick={handleInsert}>
              Insert
            </ActionButton>
            <ActionButton variant="secondary" onClick={handleRemove}>
              Remove
            </ActionButton>
          </div>
        </Popover.Content>
